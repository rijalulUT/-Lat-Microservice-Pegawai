...
func RegisterConsul() {
	...
	register.ID = "pegawai-microservice"
	register.Name = "pegawai-microservice"
	register.Address = "localhost"
	register.Port = port
	register.Check = new(consulapi.AgentServiceCheck)
	register.Check.HTTP = fmt.Sprintf("http://%s:%v/healthcheck", "localhost", port)
